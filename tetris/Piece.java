...
public class Piece {
	...
	private int[] computeSkirt() {
		...
		int[] cSkirt = new int[pointMap.size()];
		for (int i = 0; i < pointMap.size(); i++) {
			cSkirt[i] = pointMap.get(i);
		}
		return cSkirt;
	}
	...
	*/
	...
	public Piece computeNextRotation() {
		TPoint[] body_copy = new TPoint[body.length];
		...
		for (int i = 0; i < body.length; i++) {
			...
			new_x += height - 1;
			new_y = old_x;
			body_copy[i] = new TPoint(new_x, new_y);
		}
		return new Piece(body_copy);
	}
	...
	*/
	public boolean equals(Object obj) {
		...
		HashSet<TPoint> piece1 = new HashSet<TPoint>();
		for (TPoint p: body) {
			piece1.add(p);
		}
		
		HashSet<TPoint> piece2 = new HashSet<TPoint>();
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		...
		Piece temp = (Piece) root;
		
		while (true) {
			temp.next = temp.computeNextRotation();
			if (temp.next.equals(root)) break;
